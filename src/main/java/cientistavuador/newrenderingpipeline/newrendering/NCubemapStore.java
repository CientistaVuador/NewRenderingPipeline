...
public class NCubemapStore {
    ...
    public static void writeCubemap(NCubemap cubemap, OutputStream out) throws IOException {
        ...
        properties.setProperty("info.parallaxCorrected", Boolean.toString(cubemap.isParallaxCorrected()));

        if (cubemap.isParallaxCorrected()) {
            NCubemapBox info = cubemap.getCubemapBox();
            ...
            properties.setProperty("info.parallaxPositionX", Double.toString(info.getPosition().x()));
            properties.setProperty("info.parallaxPositionY", Double.toString(info.getPosition().y()));
            properties.setProperty("info.parallaxPositionZ", Double.toString(info.getPosition().z()));

            properties.setProperty("info.parallaxRotationX", Float.toString(info.getRotation().x()));
            properties.setProperty("info.parallaxRotationY", Float.toString(info.getRotation().y()));
            properties.setProperty("info.parallaxRotationZ", Float.toString(info.getRotation().z()));
            properties.setProperty("info.parallaxRotationW", Float.toString(info.getRotation().w()));

            properties.setProperty("info.parallaxHalfExtentsX", Float.toString(info.getHalfExtents().x()));
            properties.setProperty("info.parallaxHalfExtentsY", Float.toString(info.getHalfExtents().y()));
            properties.setProperty("info.parallaxHalfExtentsZ", Float.toString(info.getHalfExtents().z()));
        }
    ...
    public static NCubemap readCubemap(InputStream in) throws IOException {
        ...
        NCubemapBox info = null;
        if (Boolean.parseBoolean(properties.getProperty("info.parallaxCorrected"))) {
            info = new NCubemapBox(
                    new Vector3d(
